...
% begin code ----------------------------------------
<<loadstepNorm,eval=TRUE,echo=TRUE>>= 
library(stepNorm)
data(swirl) 
maNsamples(swirl)
maNspots(swirl)
...
% begin code ----------------------------------------
<<withinswirl,eval=FALSE,echo=TRUE>>= 
...
% begin code ----------------------------------------
<<stepSwirl1,eval=TRUE,echo=TRUE>>= 
step.swirl1 <- stepWithinNorm(swirl[,1])
# normalized data of marrayNorm class
norm.swirl <- step.swirl1[[1]]
# table of stepwise information
step.swirl1[[2]]
@  
% end code -----------------------------------------

<<eval=TRUE,echo=FALSE,results=hide>>=
d1 <- withinNorm(swirl[,1],norm="loess")
d2 <- withinNorm(d1,norm="medianPrintTip")
d3 <- withinNorm(d2,norm="medianPlate")
@ 

<<swirlPlot,include=FALSE,fig=TRUE,width=10,height=15,echo=FALSE>>=
par(mfrow=c(3,2))
maPlot(swirl[,1], main = "Swirl slide 1: pre-A normalization MA--plot", pch = ".")
maPlot(d1, main = "Swirl slide 1: post-A normalization MA--plot", pch = ".")
maBoxplot(d1, main = "Swirl slide 1: pre-PT normalization boxplot")
abline(h=0, col="gray", lwd=2, lty=2)
maBoxplot(d2, main = "Swirl slide 1: post-PT normalization boxplot")
abline(h=0, col="gray", lwd=2, lty=2)
maBoxplot(d2, x="maCompPlate", main = "Swirl slide 1: pre-PL normalization boxplot")
abline(h=0, col="gray", lwd=2, lty=2)
maBoxplot(d3, x="maCompPlate", main = "Swirl slide 1: post-PL normalization boxplot")
abline(h=0, col="gray", lwd=2, lty=2)
@
% end code ------------------------------------------------------------
...
% begin code ----------------------------------------
<<seqSwirl1,eval=TRUE,echo=TRUE>>= 
step.swirl1 <- seqWithinNorm(swirl[,1], A="loess", PT="median", PL="median", Spatial2D="none")
@  
% end code -----------------------------------------
...
PL <- list(med = fitWithin(z.fun="maCompPlate", fun="medfit"),
          rlm = fitWithin(z.fun="maCompPlate", fun="rlmfit"),
          loess = fitWithin(z.fun="maCompPlate", fun="loessfit"))
...
% begin code ----------------------------------------
<<eval=TRUE,echo=TRUE>>= 
step.swirl1 <- seqWithinNorm(swirl[,1], A="loess", PT="median", PL="none", Spatial2D="none")
@  
% end code -----------------------------------------
